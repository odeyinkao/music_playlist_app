...
import { UPDATE_USER_PLAN } from '../../graphql/mutations'
import { useDispatch } from 'react-redux'
import { setModal, SET_PLAYLIST, updateUserPlanAction } from '../../store/actions'
...
const Pricing = () => {
  ...
  const [updatePlan, {error, loading}] = useMutation(UPDATE_USER_PLAN, {
    onCompleted: (result) => {
      console.log(result);
      if(result) {          
        dispatch(updateUserPlanAction(result.me.plan))
        dispatch({
            type: SET_PLAYLIST,
            playlist: {
              musics: []
        ...
        dispatch(setModal(true, "Subscription successfull!"))
        navigate(-1)
      }
    }
  })
                ...
                <div key={tier.name} className="flex flex-col rounded-lg shadow-lg overflow-hidden">
                  ...
                  <div className="flex-1 flex flex-col justify-between px-6 pt-6 pb-8 bg-gray-50 space-y-6 sm:p-10 sm:pt-6">
                    ...
                    <div className="rounded-md shadow">
                      <button
                        className="flex w-full items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md text-white bg-gray-800 hover:bg-gray-900"
                        aria-describedby="tier-standard"
                        onClick={() => updatePlan({variables: {planId: tier.id}})}
                      >
                        Get started
                      </button>
                    </div>
